...
solution(Houses) :-
    ...
    member(house(,brit,red,), Houses),
    
    % The Swede keeps dogs
    member(house(,swede,,dog), Houses),
    
    % The Dane lives in the first house
    N1 = dane,
    
    % The green house is on the left of the white house
    left_of(house(,,green,), house(,,white,), Houses).
...
% ?- solution(H), member(house(,,_,fish), H). % Who owns the fish?`,
    description: "A logic puzzle solver demonstrating Prolog's power in solving constraint satisfaction problems.",
    sampleQueries: [
      "?- solution(Houses).",
      "?- solution(H), member(house(,Owner,,fish), H).",
      "?- solution(H), member(house(,,green,Pet), H).",
      "?- solution(H), member(house(Num,brit,,), H)."
    ]
  },
